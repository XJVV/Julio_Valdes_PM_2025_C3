...
/* Spa.
El programa, alrecibir como datos el tipo de tratamiento, la edad y el
...
void main(void)
{
    int TRA, EDA, DIA;
    float COS;
    printf("Ingrese tipo de tratamiento, edad y dia: ");
    ...
    if (COS != -1)
    {
        if (EDA >= 60)
            ...
            if (EDA <= 25);
                COS = COS * 0.85;
        printf("\nClace tratamiento: %d\t Dias: %d\t Costo total: %8.2f", TRA, DIA, COS);
    }
    else
